import {
  Compass, ClipboardCheck, PenLine, Upload, PlayCircle, Library,
  Crown, Palette, ClipboardList, Code2, Database,
  CalendarClock, BatteryLow, Megaphone, Handshake,
  Users, GitBranch, Sparkles,
  Heart, Flame, Target, MessageSquare, Zap, Repeat,
  Mail, ArrowUpRight, Download,
  type LucideIcon,
} from "lucide-react";

const map: Record<string, LucideIcon> = {
  compass: Compass,
  quiz: ClipboardCheck,
  pen: PenLine,
  upload: Upload,
  play: PlayCircle,
  library: Library,
  crown: Crown,
  palette: Palette,
  clipboard: ClipboardList,
  code: Code2,
  database: Database,
  calendar: CalendarClock,
  battery: BatteryLow,
  megaphone: Megaphone,
  handshake: Handshake,
  users: Users,
  gitbranch: GitBranch,
  sparkles: Sparkles,
  heart: Heart,
  flame: Flame,
  target: Target,
  message: MessageSquare,
  zap: Zap,
  repeat: Repeat,
  mail: Mail,
  arrow: ArrowUpRight,
  download: Download,
};

// lucide dropped brand marks, so these two are inline.
const brands: Record<string, React.ReactNode> = {
  linkedin: (
    <path d="M20.45 20.45h-3.56v-5.57c0-1.33-.02-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05a3.74 3.74 0 0 1 3.37-1.85c3.6 0 4.27 2.37 4.27 5.46v6.28zM5.34 7.43a2.07 2.07 0 1 1 0-4.13 2.07 2.07 0 0 1 0 4.13zm1.78 13.02H3.55V9h3.57v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.73v20.54C0 23.22.79 24 1.77 24h20.45c.98 0 1.78-.78 1.78-1.73V1.73C24 .77 23.2 0 22.22 0z" />
  ),
  github: (
    <path d="M12 .3a12 12 0 0 0-3.79 23.4c.6.1.82-.26.82-.58v-2.23c-3.34.72-4.04-1.42-4.04-1.42-.55-1.39-1.34-1.76-1.34-1.76-1.09-.74.08-.73.08-.73 1.2.09 1.84 1.24 1.84 1.24 1.07 1.83 2.81 1.3 3.5 1 .1-.78.42-1.31.76-1.61-2.67-.3-5.47-1.33-5.47-5.93 0-1.31.47-2.38 1.24-3.22-.14-.3-.54-1.52.1-3.18 0 0 1-.32 3.3 1.23a11.5 11.5 0 0 1 6 0c2.28-1.55 3.29-1.23 3.29-1.23.65 1.66.24 2.88.12 3.18a4.65 4.65 0 0 1 1.23 3.22c0 4.61-2.8 5.63-5.48 5.92.43.37.81 1.1.81 2.22v3.29c0 .32.22.7.83.58A12 12 0 0 0 12 .3z" />
  ),
};

export default function Icon({
  name,
  className = "w-5 h-5",
  strokeWidth = 1.75,
}: {
  name: string;
  className?: string;
  strokeWidth?: number;
}) {
  if (brands[name]) {
    return (
      <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
        {brands[name]}
      </svg>
    );
  }
  const Cmp = map[name] ?? Sparkles;
  return <Cmp className={className} strokeWidth={strokeWidth} aria-hidden="true" />;
}
